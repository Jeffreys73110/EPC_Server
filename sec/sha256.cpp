...
int main(){
	int i;
	char key_c[]="06dab76d6e2677675f6683ce15ba2bbb32d6fa48880930a6bbe43795bf5ffe4a";
	char input_c[]="1000f11000033ca061cb46c40006";
	uint8_t key[32],input[14],kasme[32],knasint[32],knasenc[32];
	for(i=0;i<32;i++)
		key[i]=c2u(key_c[i*2])*16+c2u(key_c[i*2+1]);
	for(i=0;i<14;i++)
		input[i]=c2u(input_c[i*2])*16+c2u(input_c[i*2+1]);
	sha256_hmac(key,32,input,14,kasme,0);

	char input_cc[]="15020001010001";
	for(i=0;i<7;i++)
		input[i]=c2u(input_cc[i*2])*16+c2u(input_cc[i*2+1]);
	sha256_hmac(kasme,32,input,7,knasint,0);

	char input_ccc[]="15010001010001";
	for(i=0;i<7;i++)
		input[i]=c2u(input_ccc[i*2])*16+c2u(input_ccc[i*2+1]);
	sha256_hmac(kasme,32,input,7,knasenc,0);
	for(i=0;i<32;i++)
		printf("%02x",knasenc[i]);
	printf("\n");
}*/
