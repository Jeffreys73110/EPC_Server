...
int main(){
	int i;
	char key_c[]="2a0e66997177052319caafe0a3994cc7c24199fb0578cacd3f59c891c8ad4cc0";
	char input_c[]="00075d010005e060c04070";
	u8 key[32],input[14],output[32];
	u8* ss;
	for(i=0;i<32;i++)
		key[i]=c2u(key_c[i*2])*16+c2u(key_c[i*2+1]);
	for(i=0;i<11;i++)
		input[i]=c2u(input_c[i*2])*16+c2u(input_c[i*2+1]);
	ss=snow3g_f9(key,0,0,1,input,88);
	printf("MAC: ");
	for(i=0;i<4;i++)printf("%02x",ss[i]);
	printf("\n");
}*/

static const u8 S[256] = { 99,124,119,123,242,107,111,197, 48,  1,103, 43,254,215,171,118,
...
u8 s3g_mul_x_pow(u8 v, u8 i, u8 c) {
    if (i == 0)
        return v;
    else
...
u32 s3g_mul_alpha(u8 c) {
    return ((((u32) s3g_mul_x_pow(c,  23, 0xa9)) << 24) |
            (((u32) s3g_mul_x_pow(c, 245, 0xa9)) << 16) |
            (((u32) s3g_mul_x_pow(c,  48, 0xa9)) <<  8) |
            (((u32) s3g_mul_x_pow(c, 239, 0xa9))));
}
u32 s3g_div_alpha(u8 c) {
    return ((((u32) s3g_mul_x_pow(c, 16, 0xa9)) << 24) |
            (((u32) s3g_mul_x_pow(c, 39, 0xa9)) << 16) |
            (((u32) s3g_mul_x_pow(c,  6, 0xa9)) <<  8) |
            (((u32) s3g_mul_x_pow(c, 64, 0xa9))));
}
u32 s3g_s1(u32 w) {
    u8 r0 = 0, r1 = 0, r2 = 0, r3 = 0;
    u8 srw0 = S[(u8) ((w >> 24) & 0xff)];
    u8 srw1 = S[(u8) ((w >> 16) & 0xff)];
    u8 srw2 = S[(u8) ((w >>  8) & 0xff)];
    u8 srw3 = S[(u8) ((w)       & 0xff)];
...
void s3g_clock_lfsr(S3G_STATE * state, u32 f) {
    u32 v = (
            ((state->lfsr[0] << 8) & 0xffffff00) ^
            (s3g_mul_alpha((u8) ((state->lfsr[0] >> 24) & 0xff))) ^
            (state->lfsr[2]) ^
            ((state->lfsr[11] >> 8) & 0x00ffffff) ^
            (s3g_div_alpha((u8) ((state->lfsr[11]) & 0xff))) ^
            (f)
            );
    u8 i;
...
u32 s3g_clock_fsm(S3G_STATE * state) {
    u32 f = ((state->lfsr[15] + state->fsm[0]) & 0xffffffff) ^
               state->fsm[1];
    u32 r = (state->fsm[1] + (state->fsm[2] ^ state->lfsr[5])) &
               0xffffffff;
...
void s3g_initialize(S3G_STATE * state, u32 k[4], u32 iv[4]) {
    u8 i = 0;
    u32 f = 0x0;

    state->lfsr = (u32 *) calloc(16, sizeof(u32));
    state->fsm  = (u32 *) calloc( 3, sizeof(u32));
...
void s3g_generate_keystream(S3G_STATE * state, u32 n, u32 *ks) {
    u32 t = 0;
    u32 f = 0x0;
...
void zero_tailing_bits(u8 * data, u32 length_bits) {
    u8 bits = (8 - (length_bits & 0x07)) & 0x07;
    data[(length_bits + 7) / 8 - 1] &= (u8) (0xFF << bits);
}
void encryption_eea1(u8  *key,u32 count,u8 bearer,u8 direction,u8 *msg,u32 msg_len,u8 *out)
{
    ...
    u32 *ks;
    int32_t i;
    u32 msg_len_block_8, msg_len_block_32, m;
        ...
        out != NULL)
    {
        ...
        ks = (u32 *) calloc(msg_len_block_32, sizeof(u32));
